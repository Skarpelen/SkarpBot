...
//            => await RespondAsync(echo + (mention ? Context.UserId.Mention : string.Empty));
...
//            => await RespondAsync(text: $":wave: {Context.UserId} said hi to you, <@{user.id}>!");
...
//            channel = channel ?? (Context.UserId as IGuildUser)?.VoiceChannel;
//            if (channel == null) { await Context.Channel.SendMessageAsync("UserId must be in a voice channel, or a voice channel must be passed as an argument."); return; }
...
//        {
//            await DataAccessLayer.MenuPicker(Context.UserId.id, Context.guild.id, value);
...
//        {
//            string answer = await DataAccessLayer.EquipWeapon(Context.UserId.id, Context.guild.id);
...
//        {
//            string answer = await DataAccessLayer.DestroyInventory(Context.UserId.id, Context.guild.id);
...
//            await DataAccessLayer.MenuPicker(Context.UserId.id, Context.guild.id, text);
